package pl.itgolo.apps.sur.Modules.CompanyCatalogue.PreConfig.Step4SeoConfig.Abstracts;
 ...
 */
public abstract class SeoConfig implements IStepCompanyCatalogue{
    ...
    public void launchStep() throws StepCompanyCatalogueException {
        ...
        if (BooleanUtils.isNullOrFalse(setConfiguration())) {
            throw new StepCompanyCatalogueException("Failed set configuration in step 'Seo config'");
        }
        LogService.debug("Launch validate set configuration");
        if (BooleanUtils.isNullOrFalse(validateSetConfiguration())) {
            throw new StepCompanyCatalogueException("Failed validate set configuration in step 'Seo config'");
        }
